...
def getAllImages(input=None,pagina="1"):
    if input is None:
        if pagina == "":
            json_response = requests.get(config.DEFAULT_REST_API_URL + "1").json()
        else:
            json_response = requests.get(config.DEFAULT_REST_API_URL + pagina).json()
    else:
